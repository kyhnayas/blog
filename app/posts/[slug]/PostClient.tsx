  ...
  const handleFollowToggle = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        setMessage({
          type: "success",
          text: `You are now following @${author.username}!`,
        });
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white font-sans py-10 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-white transition-colors mb-8"
        >
              ...
              {firstHalfContent.split("\n").map((line: string, i: number) => {
                if (line.startsWith("# "))
                  return (
                    <h1
                      key={i}
                      className="text-3xl font-extrabold mt-6 mb-3 font-sans"
                    >
                      {line.replace("# ", "")}
                    </h1>
                  );
                if (line.startsWith("## "))
                  return (
                    <h2
                      key={i}
                      className="text-2xl font-bold mt-5 mb-3 font-sans"
                    >
                      {line.replace("## ", "")}
                    </h2>
                  );
                if (line.startsWith("### "))
                  return (
                    <h3
                      key={i}
                      className="text-xl font-bold mt-4 mb-2 font-sans"
                    >
                      {line.replace("### ", "")}
                    </h3>
                  );
                if (line.startsWith("> "))
                  return (
                    <blockquote
                      key={i}
                      className="border-l-4 border-primary-500 pl-4 italic text-zinc-500 my-4 font-sans"
                    >
                      {line.replace("> ", "")}
                    </blockquote>
                  );
                return (
                  <p
                    key={i}
                    className="my-4 text-zinc-800 dark:text-zinc-300 leading-relaxed min-h-[1rem]"
                  >
                    {line}
                  </p>
                );
            ...
            <div className="rounded-3xl border border-zinc-200/50 dark:border-zinc-800/60 p-6 sm:p-8 bg-zinc-50 dark:bg-zinc-900/20 flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              <div className="space-y-3 text-center sm:text-left flex-grow">
                ...
                <p className="text-xs sm:text-sm text-zinc-650 dark:text-zinc-400 leading-relaxed">
                  {author.bio ||
                    "This editor covers high-CPM contents and technical stories. Follow them to stay updated on future releases."}
                </p>
                {message && (
