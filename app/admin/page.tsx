...
import { Shield, Users, Award, Search, ArrowLeft, Loader2, AlertCircle, CheckCircle, UserCheck, UserMinus, Sparkles } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-zinc-400 text-sm font-medium">Authorizing & loading admin control panel...</span>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-zinc-400">Promote readers to Editors, manage platform users, and review ad units.</p>
          </div>

          {/* Quick Metrics */}
          <div className="flex items-center gap-4">
            <div className="px-4 py-2 rounded-2xl bg-zinc-900 border border-zinc-800 text-center">
              <span className="block text-xs font-bold text-zinc-500 uppercase">Total Users</span>
              <span className="text-xl font-extrabold text-white">{profiles.length}</span>
            </div>
            <div className="px-4 py-2 rounded-2xl bg-zinc-900 border border-zinc-800 text-center">
              <span className="block text-xs font-bold text-zinc-500 uppercase">Editors</span>
              <span className="text-xl font-extrabold text-emerald-400">{profiles.filter(p => p.role === 'editor').length}</span>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-zinc-800/30 text-sm">
                {filteredProfiles.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-12 px-6 text-center text-zinc-500 font-medium">
                    ...
                    <tr key={p.id} className="hover:bg-zinc-900/25 transition-colors">
                      ...
                      <td className="py-4.5 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-semibold text-white block">{p.full_name || 'Anonymous User'}</span>
                            <span className="text-[11px] text-zinc-500 block">ID: {p.id.slice(0, 8)}...</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4.5 px-6">
                        {p.adsense_pub_id ? (
                          <div className="space-y-0.5 text-xs text-emerald-400 font-mono font-medium">
                            <span className="block">PUB: {p.adsense_pub_id}</span>
                            {p.adsense_slot_id && <span className="block text-zinc-400">SLOT: {p.adsense_slot_id}</span>}
                          </div>
                        ) : (
                          <span className="text-xs text-zinc-500 italic">No ads connected</span>
                        )}
                      </td>
